...
public class Level2 {
	...
	long nbr = employees.stream().filter(e->e.getName().startsWith("s")).count();
	...
	List<Employee> emps = employees.stream().collect(Collectors.toList());
		...
		List<Employee> emps2 = employees.stream().filter(e->e.getName().startsWith("s")).collect(Collectors.toList());/* TO DO 5 */;
	...
	*/
	...
	/*Second way*/
		employees.stream().filter(e->e.getName().startsWith("s")).findFirst().orElse(null);
		
	/* TO DO 8: Afficher le second employ� dont le nom commence avec s */
	employees.stream().filter(e->e.getName().startsWith("s")).skip(1).findFirst().orElse(null);
